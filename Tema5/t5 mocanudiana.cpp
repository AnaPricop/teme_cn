...
public:
    void patrat(double lungime, int nivel, CPunct& p, CVector v) {
        ...
        // impartim in grid de 3x3 si 
        // eliminam pe cel central (1,1)
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) {
                if (!(j == 1 && i == 1)) {
                    ...
                    CPunct p(x + j * lungime / 3, y + i * lungime / 3);
                    patrat(lungime / 3, nivel - 1, p, v);
                }
            }
        }
    }
    ...
    void afisare(double lungime, int nivel) {
        ...
        patrat(lungime, nivel, p, v);
    }
};

class ArborePerronInv {
public:
    void arborePerron(double lungime, int nivel, double factor, CPunct p, CVector v){
        CPunct p1, p2;
        if (nivel == 0) {
        }
        else {
            ...
            v.deseneaza(p, lungime);
            p1 = v.getDest(p, lungime);
            arborePerron(lungime * factor, nivel - 1, factor, p1, v);
    ...
    //folosim arborele perron si il generam invers
    void afisare(double lungime, int nivel) {
        CVector v(0.0, -1.0);
        CPunct p(-1, 2.0);
...
class TriunghiSierpinski {
public:
    void triunghi_sierpinski(double lungime, int nivel, CPunct& p, CVector v, int dir) {
        ...
        if (nivel == 0) {
            ...
            //impartim in 3 segmente la 60 de grade
            ...
            v.rotatie(dir * 60);
            triunghi_sierpinski(lungime / 2, nivel - 1, p, v, -dir);
            v.rotatie(-dir * 60);
            triunghi_sierpinski(lungime / 2, nivel - 1, p, v, dir);
            v.rotatie(-dir * 60);
            triunghi_sierpinski(lungime / 2, nivel - 1, p, v, -dir);
    ...
    void afisare(double lungime, int nivel) {
        ...
        triunghi_sierpinski(lungime, nivel, p, v, 1);
    }
};
...
// afisare covorul lui sierpinski
void Display7() {
    CovorSierpinski sierpinski;
    sierpinski.afisare(1, nivel);
    char c[3];
    sprintf(c, "%2d", nivel);
    glRasterPos2d(-0.98, -0.98);
    glutBitmapCharacter(GLUT_BITMAP_9_BY_15, 'N');
...
//afisare arbore perron invers
void Display8() {
    ArborePerronInv perron;
    perron.afisare(1, nivel);
    char c[3];
    sprintf(c, "%2d", nivel);
    glRasterPos2d(-0.98, -0.98);
    glutBitmapCharacter(GLUT_BITMAP_9_BY_15, 'N');
...
//afisare triunghiul lui sierpinski
void Display9() {
    TriunghiSierpinski sierpinski;
    sierpinski.afisare(1, nivel);
    char c[3];
    sprintf(c, "%2d", nivel);
    glRasterPos2d(-0.98, -0.98);
...
std::pair<int, bool> isMandelbrot(CComplex c) {

    static const int MAX_ITERATION = 100;
    double epsilon = 0.5;
    
    if (c.getModul() < epsilon) {
        return std::make_pair(0, true);
    }
    int iteratii = 0;
    CComplex z = CComplex(0.0, 0.0); //z0
    CComplex z_anterior;
    while (true) {
        iteratii++;
        z_anterior = z;
        // generam noul z
        z = z_anterior.pow2() + c;
        // daca modul mai mare ca 2 -> oprim interatiile
        if (z.getModul() > 2) {
            return std::make_pair(iteratii, false);
        }
       
        else if (iteratii > MAX_ITERATION) {
            return std::make_pair(iteratii, true);
        }
    }
}
...
void DisplayM() {
    ...
    colors.push_back({ 1.0f, 0.5f, 1.0f });
    ...
    for (double y = -2.0; y <= 2.0; y += step) {
        for (double x = -2.0; x <= 2.0; x += step) {
            CComplex c(x, y);
            std::pair<int, bool> result = isMandelbrot(c);

            if (result.second) {
                glColor3f(1, 0, 0); // rosu
            }
            else {
                int iteratii = result.first;
                int colorIdx = iteratii;
                if (colorIdx >= colors.size()) {
                    colorIdx = colors.size() - 1;
                }
                auto color = colors[colorIdx];
                glColor3f(color[0], color[1], color[2]); // rosu
            }
