...
def jacobi_method(a, b, d, len):
    x_current = [0 for _ in range(len)]
    x_previous = [0 for _ in range(len)]
    k = 0
    delta_x = eps
    k_max = 10000
    while delta_x >= eps and k <= k_max and delta_x <= 1e+8:
        x_previous = x_current
        x_current = step_formula(x_previous, a, b, d, len)
        delta_x = np.linalg.norm(np.subtract(x_current, x_previous))
        k = k + 1
    if delta_x < eps:
        print('Iterations:', k)
        return x_current
...
def step_formula(x_prev, a, b, d, size):
    ...
    for i in range(size):
        for element in a[i]:
            ...
            j = element[1]
            if i != j:
                x[i] -= value * x_prev[j]  # sum1
                x[j] -= value * x_prev[i]  # sum2
